...
def load_mnist(path, kind='train', normalize=True, return_4d_tensor=True, one_hot=True):
    ...
    if return_4d_tensor:
        # 28x28 is specific to MNIST, change for other data
...
def split_and_select_random_data(x, y, xtest, ytest, num_target_classes=5, num_examples_per_class=1):
    ...
    x_target_labeled, y_target, x_target_unlabeled = get_random_labeled_examples(x_target_all,
                                                                                 y_target_all,
                                                                                 target_classes[0],
                                                                                 num_examples_per_class)

    for c in target_classes[1:]:
        x_temp, y_temp, x_unlabeled_temp = get_random_labeled_examples(x_target_all,
                                                                       y_target_all,
                                                                       c,
                                                                       num_examples_per_class)
...
def freeze_layers(model, num_fixed_layers, count_only_trainable_layers=True,
                  reinitialize_remaining=False, reinitialize_last=False):
